...
class BreadService extends Controller
{
    use BreadRelationshipParser;
    use AuthorizesRequests;

    public function browse($request, $slug)
    {
        if(!$this->hasPermission('browse', $slug)) {
            return "QAQ";
        }
    ...
    {
        if(!$this->hasPermission('read', $slug)) {
            return "QAQ";
        }
    ...
    {
        if(!$this->hasPermission('edit', $slug)) {
            return "QAQ";
        }
    ...
    {
        if(!$this->hasPermission('add', $slug)) {
            return "QAQ";
        }
    ...
    public function destroy($request, $id, $slug)
    {
        if(!$this->hasPermission('delete', $slug)) {
            return "QAQ";
        }
    ...
    private function hasPermission($action, $target)
    {
        $work = $action . '_' . $target;
        return Auth::check() && Voyager::can($work);
    }
}
